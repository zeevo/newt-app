...
export async function doInit(options: Options) {
  const groupOpts = {
    ...
    ...(!options.ci && {
      ...
      singleProcess: () =>
        p.confirm({
          message: "Single-process mode? (Next.js static export served by NestJS in production)",
          initialValue: false,
        }),
    }),
  };
  ...
  try {
    ...
    const allModules = [
      ...
      ...(useSingleProcess ? [templates.singleProcessStaticExport] : []),
    ];
...
program
  ...
  .option("--single-process", "Single-process mode: Next.js static export served by NestJS in production (used with --ci)", false)
  .action(
    async (
      name: string,
      options: {
