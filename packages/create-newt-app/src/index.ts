...
export async function doInit(options: Options) {
  const groupOpts = {
    ...
    ...(!options.ci && {
      ...
      singleProcess: () =>
        p.confirm({
          message: "Single-process mode? (Next.js + NestJS share one process via instrumentation.ts)",
          initialValue: false,
        }),
    }),
  };
  ...
  try {
    ...
    const allModules = [
      ...
      ...(useSingleProcess ? [templates.singleProcessInstrumentation] : []),
    ];
...
program
  ...
  .option("--single-process", "Single-process mode: Next.js + NestJS share one process via instrumentation.ts (used with --ci)", false)
  .action(
    async (
      name: string,
      options: {
