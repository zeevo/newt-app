...
export async function doInit(options: Options) {
  const groupOpts = {
    ...
    ...(!options.ci && {
      ...
      singleProcess: () =>
        p.confirm({
          message: "Single-process mode? (Next.js + NestJS share one server)",
          initialValue: false,
        }),
    }),
  };
  ...
  try {
    ...
    const allModules = [
      ...
      ...(useSingleProcess ? [templates.singleProcessCustomServer] : []),
    ];
...
program
  ...
  .option("--single-process", "Single-process mode: Next.js + NestJS share one server (used with --ci)", false)
  .action(
    async (
      name: string,
      options: {
