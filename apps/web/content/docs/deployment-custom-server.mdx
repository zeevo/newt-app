...
`/api/auth/*` is carved out and sent to Next.js so that better-auth's own route handler (`app/api/auth/[...all]/route.ts`) continues to work — the same as in the default setup where `afterFiles` rewrites only apply after internal route handlers are checked.

```
apps/web/server.ts   →  port 3000  (routes to Next.js or NestJS)
```
