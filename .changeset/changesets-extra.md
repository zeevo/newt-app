...
Scaffolds `.changeset/config.json` and its README, adds `@changesets/cli` to the root devDependencies, and wires `changeset`, `version-packages` and `release` scripts. Combines with `anti-slop`, and unlike it carries no linter requirement, so the extras prompt now appears for both linters.
